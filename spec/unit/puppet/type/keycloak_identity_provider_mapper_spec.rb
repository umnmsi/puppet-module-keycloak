...
describe Puppet::Type.type(:keycloak_identity_provider_mapper) do
  ...
  it 'handles composite name' do
    component = described_class.new(name: 'foo for saml on test')
    expect(component[:name]).to eq('foo for saml on test')
    expect(component[:resource_name]).to eq('foo')
    expect(component[:identity_provider]).to eq('saml')
    expect(component[:realm]).to eq('test')
  end

  it 'handles composite name with space' do
    component = described_class.new(name: 'full name for saml on test')
    expect(component[:name]).to eq('full name for saml on test')
    expect(component[:resource_name]).to eq('full name')
    expect(component[:identity_provider]).to eq('saml')
    expect(component[:realm]).to eq('test')
  end

  it 'defaults to type=saml-user-attribute-idp-mapper' do
    expect(resource[:type]).to eq('saml-user-attribute-idp-mapper')
  end

  it 'allows valid type' do
    config[:type] = 'hardcoded-attribute-idp-mapper'
    expect(resource[:type]).to eq('hardcoded-attribute-idp-mapper')
  end
