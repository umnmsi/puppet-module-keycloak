...
describe Puppet::Type.type(:keycloak_identity_provider_mapper).provider(:kcadm) do
  let(:type) do
    Puppet::Type.type(:keycloak_identity_provider_mapper)
  end
  let(:resource) do
    type.new(name: 'foo',
             realm: 'test',
  ...
  describe 'self.instances' do
    it 'creates instances' do
      ...
      allow(described_class).to receive(:kcadm).with('get', 'identity-provider/instances', 'master').and_return(my_fixture_read('get-master-idp.out'))
      allow(described_class).to receive(:kcadm).with('get', 'identity-provider/instances/saml/mappers', 'master').and_return(my_fixture_read('get-master-idp-saml.out'))
      allow(described_class).to receive(:kcadm).with('get', 'identity-provider/instances/saml2/mappers', 'master').and_return(my_fixture_read('get-master-idp-saml2.out'))
      allow(described_class).to receive(:kcadm).with('get', 'identity-provider/instances', 'test').and_return(my_fixture_read('get-test-idp.out'))
      allow(described_class).to receive(:kcadm).with('get', 'identity-provider/instances/saml/mappers', 'test').and_return(my_fixture_read('get-test-idp-saml.out'))
    ...
    it 'returns the resource for a fileset' do
      ...
      expect(property_hash[:name]).to eq('mail for saml on master')
    end
  end
